...
namespace LeechCraft
{
	namespace Plugins
	{
		namespace Syncer
		{
			namespace
			{
				QByteArray FmtMsg (const QList<QByteArray>& lists)
				{
					QByteArray msg;
					{
				...
				QList<QByteArray> UnfmtMsg (const QByteArray& msg)
				{
					QDataStream ds (msg);
			...
			ServerConnection::ServerConnection (const QString& chain, QObject *parent)
			: QObject (parent)
			...
			void ServerConnection::reqMaxDelta ()
			{
				...
				lists << "MAXDELTA" << Chain_.toLatin1 ();
